...
import numpy as np
from ceyin.loss import cal_gradient_penalty
...
class LossWithGAN_STE(nn.Module):
    def __init__(self, args, extractor, disc, Lamda, lr, betasInit=(0.5, 0.9)):
        ...
        # self.lamda = Lamda  没用到
        self.paste = args.PasteImage  # paste
        self.mask_sigmoid = args.mask_sigmoid  # 对mask分支的输出是否进行sigmoid
    ...
    def forward(self, input, mask_gt, x_o1, x_o2, x_o3, output, mm, gt, raw_mask):
        # input=img:加了text mask的图,
        # mask_gt：text mask
        # output：精细网络的输出图
        # gen_mask=mm：mask分支输出，其实是文字定位的输出
        # gt=real_B:gt图
        # raw_mask：原始的mask图
...
#         requires_grad(self.discriminator, True)
        ...
        mask = mask_gt  # 后续加上的text mask
        # mask = mm
        if self.paste:  # paste是针对mask区域，和gt不构成一对
            output = mask * input + (1 - mask) * output
        else:
            output = raw_mask * output + (1 - raw_mask) * input
            # 1-raw_mask:原始文字区域，raw_mask*output:精细图输出上的除了原始mask的其他区域，这里
            # 组的gt图是带有原始mask的图，1-raw_mask*input,在input上拿了原始的text mask，
            # 也就说这张合成图，原始区域的text是真的，和加了text mask的组成一对

        # -----------------------------------------------------------------------------
        D_real = self.discriminator(gt, mask)  # gt=原始img
        D_fake = self.discriminator(output.detach(), mask)  # 生成图的，加了原始text
#         Real = torch.ones_like(D_fake)
#         Fake = torch.zeros_like(D_fake)
        ...
        # if flag:
        #     print("D_real: ", torch.mean(F.relu(1.+D_real)).detach().cpu().datasets, "D_fake: ", torch.mean(F.relu(1.+D_fake)).detach().cpu().datasets)
#         self.D_optimizer.zero_grad()
...
#         D_loss.backward()
        ...
        # mask：text mask,output:原始的text mask是真的，和gt组成一对。1-mask拿添加的text mask,和input中除了mask的其他区域，
        # input把添加的text mask换成output
        output_comp = mask * input + (1 - mask) * output

        # local-aware reconstruction loss
        holeLoss = 10 * self.l1((1 - mask) * output, (1 - mask) * gt) # text mask
        if self.paste:
            validAreaLoss = 0
        ...
        if self.paste:
            ...
            msrloss = 8 * self.l1((1 - mask) * x_o3, (1 - mask) * gt) + 0.8 * self.l1(raw_mask * mask * x_o3, raw_mask * mask * gt) + \
                      6 * self.l1((1 - masks_b) * x_o2, (1 - masks_b) * imgs2) + 1 * self.l1(raw_masks_b * masks_b * x_o2,
                                                                                             raw_masks_b * masks_b * imgs2) + \
                      5 * self.l1((1 - masks_a) * x_o1, (1 - masks_a) * imgs1) + 0.8 * self.l1(raw_masks_a * masks_a * x_o1,
                                                                                               raw_masks_a * masks_a * imgs1)
