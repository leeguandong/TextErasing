...
class Dis_Mask(nn.Module):
    def __init__(self, n_in_channel=3):
        super(Dis_Mask, self).__init__()
        self.mask_deconv_a = DeConvWithActivation(256, 256, kernel_size=3, padding=1, stride=2)
        self.mask_conv_a = ConvWithActivation(256, 128, kernel_size=3, padding=1, stride=1)
        self.mask_deconv_b = DeConvWithActivation(128, 128, kernel_size=3, padding=1, stride=2)
        self.mask_conv_b = ConvWithActivation(128, 64, kernel_size=3, padding=1, stride=1)
        self.mask_deconv_c = DeConvWithActivation(64, 64, kernel_size=3, padding=1, stride=2)
        self.mask_conv_c = ConvWithActivation(64, 32, kernel_size=3, padding=1, stride=1)
        self.mask_deconv_d = DeConvWithActivation(32, 32, kernel_size=3, padding=1, stride=2)
...
class STRnet2(nn.Module):
    ...
    def forward(self, x):
        ...
        ###refine sub-network
        x = self.coarse_conva(x_o_unet)  # 2,32,768,512
        x = self.coarse_convb(x)  # 2,64,384,256
        x = self.coarse_convc(x)  # 2,64,384,256
        x_c1 = x  ###concate feature1
        x = self.coarse_convd(x)  # 2,128,192,128
        x = self.coarse_conve(x)  # 2,128,192,128
        x = self.coarse_convf(x)  # 2,128,192,128
        x_c2 = x  ###concate feature2
        x = self.astrous_net(x)  # 2,128,192,128
        x = self.coarse_convk(x)  # 2,128,192,128
        x = self.coarse_convl(x)  # 2,128,192,128
        x = self.coarse_deconva(torch.cat([x, x_c2, self.c2(con_x2)], dim=1))  # 2,64.384,256
        x = self.coarse_convm(x)  # 2,64,384,256
        x = self.coarse_deconvb(torch.cat([x, x_c1, self.c1(con_x1)], dim=1))  # 2,32,768,512
        x = self.coarse_convn(x)  # 2,3,768,512
        # return x_o1, x_o2, x_o_unet, x, mm
        return xo0, x_o1, x_o2, x_o_unet, x, mm, x_mask
        # 2,512,24,16 / 2,64,192,128 / 2,32,384,256 / 2,3,768,512 / 2,3,768,512 / 2,3,768,512 / 2,256,48,32
