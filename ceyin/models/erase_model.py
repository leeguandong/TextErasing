...
from ceyin.models.erase.Model import VGG16FeatureExtractor
from ceyin.models.erase.discriminator import Discriminator_STE
from ceyin.models.erase.sa_gan import STRnet2, Dis_Mask, init_weights
from torch import autograd

from ceyin.models.erase.Loss import LossWithGAN_STE
...
class EraseModel(BaseModel):
    ...
    def initialize(self, opt):
        ...
        # self.visual_names.append('comp_all')
        # specify the ceyin you want to save to the disk. The training/test scripts will call <BaseModel.save_networks> and <BaseModel.load_networks>
        if self.isTrain:
...
#             self.netD = on_device(opt.online, self.netD, opt.gpu_ids)
    ...
    def set_inputs(self, input):
        # img:加过text mask的图，gt:原图，mask：text mask,raw_mask:原始的mask
    ...
    def set_specific_image(self, no):
        self.real_A = self.real_As[:, no, :]  # imgs:no=0
        self.mask = self.masks[:, no, :]  # 新添加的text mask
        if self.comp_flag:
    ...
    def forward(self):
        ...
        # comp_all：生成图上的新加的text_mask，生成图上原始的mask，真实图其他区域
        # comp_B:精细图上的text mask区域和加了text mask区域的原始图组合，此时这张图中text mask区域是生成的，其他区域是原始图的
        # comp_G:和B是一致的，只不过这里的mask是mask分支生成的
        # fake_B:精细图输出
        # gen_mask：mask分支的输出
        # mask_all:所有的mask，包括原始mask和text mask
        # mask：text mask
        # real_A:加过text mask的图
        # real_B:gt,原图
        # x_o3：粗略输出图
        ...
        self.x_o0, self.x_o1, self.x_o2, self.x_o3, self.fake_B, self.gen_mask, self.x_mask = self.netG(self.real_A)  # G(A) real_A
        # xo0,xo1,xo2,xo3是粗网络的四层输出，其中o3是粗网络的输出，fake_B:精细网络的输出，gen_mask：mask分支输出，x_mask:backbone的输出
...
#         import pdb;pdb.set_trace()
        ...
        # mask是加入的text mask(文字mask是0，其余区域是1)，1-mask是文字区域，mask是其他区域，
        # comp_B把精细网络生成的文字区域和加入了text mask的原图进行组合，此时这张图中text mask区域是生成的，其他区域是原始图的

        # 对mask区域是否进行sigmoid，1-gen_mask就是文字区域
        ...
        # comp_G:mask分支的输出其实是文字的位置，和上面的comp_B是一个意思

        if self.comp_flag:
            self.comp_all = self.fake_B * (1 - self.mask) + self.fake_B * (1 - self.raw_mask) + self.real_A * self.mask * self.raw_mask
            # 生成图上的新加的text_mask，生成图上原始的mask，真实图其他区域
...
#         import pdb;pdb.set_trace()
        ...
        if self.contrastive_loss:
            ...
            self.loss_G_contr = lam * self.dis(self.comp_B, self.real_A) / (
                1e-7 + self.dis(self.comp_B, self.real_A) + pro * max(1e-7, self.dis(self.coarse_comp_B, self.comp_B) ** ex))
            # print(G_loss, self.loss_G_conti)
        ...
        if stype == "focal":
            ...
            self.target_x_o0, _, _, _, _, _, _ = self.netG(self.real_A)
            self.loss_afn = 0.05 * (self.adaptive_feature_norm(m(self.x_o0).view(self.x_o0.size(0), -1)) + self.adaptive_feature_norm(
                m(self.target_x_o0).view(self.x_o0.size(0), -1)))
    ...
    def optimize_mask_dis(self):
        ...
        # x_mask是backbone的输出：1,256,48,32 ；D_mask:1,3,768,512,看起来像是mask分支，mask分支做就是判定位置准不准呗
        self.loss_D_mask = torch.mean(dice_loss(D_mask, 1 - self.raw_mask, sig=False)) # 1-self.raw是文本区域，d_mask是论文中的Dtext
    ...
    def update_val_real_dis(self, real_dataset, gen_configs):
        ...
        for _ in range(1): #训练了一个epoch，在验证数据集上
    ...
    def calcu_loss(self):
        ...
        # comp_B:精细图上的text mask区域和img的其他区域，G的输出图，和gt计算l1，其实在算text mask擦除的干不干净
        # # G_loss = G_loss.sum()
        # print(G_loss.shape)
        return G_loss
