...
import ceyin.utils.text_utils as tu
...
def set_font(color, size, image, x, y, direct, config):
    # ["T", 0, 2], ["F", 0, 12], ["C", 0, 3], ["S", 1, 2], ["Alpha", 0, 1],
    # ["Gau", 0, 1], ["Poi", 0, 1], ["Warp", 0, 1], ["Ital", 0, 1], ["Cur", 0, 1],
    # ["A", 0, 2], ["A1", 0, 2], ["A1_S", 0, 1], ["A1_C", 0, 1],
    # ["A2", 0, 2], ["A2_R", 6, 7], ["A2_D", 1, 2], ["A2_C", 0, 1], ["A2_G", 0, 1], ["A2_A", 0, 1]

    # 外部可调整的是20种算子
    # T:mser
    # C:color
    # S:size字体大小，对应4种不同的策略
    # F:font 字体，上面有个Font type
    # Alpha:Apha blending  透明度混合
    # Poi:Poission blending  泊松分布
    # Gau:Gaussian blurring  高斯模糊
...
def random_gen_config(space_type):
    with open("./ceyin/utils/space_config.json", "r") as f:
...
def gen_valid_config(index, space_type):
    with open("./utils/space_config.json", "r") as f:
        space = json.load(f)["range"][space_type]
    config = {}
    for param in space:
        config[param[0]] = np.random.randint(param[1], param[2])
...
def gen_config_from_parse(policys, space_type):
    configs = []
    with open("./ceyin/utils/space_config.json", "r") as f:
        space = json.load(f)["shift"][space_type] # space_type对应options中的选型和json中的数据是一致的，这里面的[]其实也是策略的选择
    for policy in policys:
        if len(policy) == 1:
            config = int(policy[0])
        else:
            # poclices和config中的关系还是一一对应的，映射关系
            config = {}
            cnt = 0
            for param in space:
                if param[1] == 0:
    ...
    return configs
#<class 'list'>: [{'T': 0, 'F': 1, 'C': 0, 'S': 0, 'Alpha': 0, 'Gau': 0, 'Poi': 1, 'Warp': 0, 'Ital': 1, 'Cur': 0, 'A': 0, 'A1': 0, 'A1_S': 0, 'A1_C': 2, 'A2': 1, 'A2_R': 0, 'A2_D': 0, 'A2_C': 2, 'A2_G': 2, 'A2_A': 2}, {'T': 1, 'F': 10, 'C': 1, 'S': 0, 'Alpha': 0, 'Gau': 0, 'Poi': 0, 'Warp': 0, 'Ital': 1, 'Cur': 1, 'A': 1, 'A1': 0, 'A1_S': 1, 'A1_C': 1, 'A2': 1, 'A2_R': 2, 'A2_D': 3, 'A2_C': 0, 'A2_G': 3, 'A2_A': 0}]
    ...
    if int(text_info["direction"]) == 1.0:
        ...
        for k, s in enumerate(sentence):
            if c_A1 == 1 and c_A2 == 1:
                draw.text((x - shadow_x, y + fh * k - shadow_y), s, fill=shadow_fill, font=font, stroke_width=stroke_width,
        ...
        if c_A1 == 1 and c_A2 == 1:
            draw.text((x - shadow_x, y - shadow_y), sentence, fill=shadow_fill, font=font, stroke_width=stroke_width,
                      stroke_fill=shadow_fill)
